...
contract TokenCreateCustomContract is HederaTokenService, ExpiryHelper, KeyHelper, FeeHelper {
    ...
    function mintTokenToAddressPublic(address token, address receiver, int64 amount, bytes[] memory metadata)
        public
        returns (int256 responseCode, int64 newTotalSupply, int64[] memory serialNumbers)
    {
    ...
    function mintNonFungibleTokenToAddressPublic(address token, address receiver, int64 amount, bytes[] memory metadata)
        public
        returns (int256 responseCode, int64 newTotalSupply, int64[] memory serialNumbers)
    {
        (responseCode, newTotalSupply, serialNumbers) = mintTokenPublic(token, amount, metadata);
