...
contract TokenCreateContract is HederaTokenService, ExpiryHelper, KeyHelper {
    ...
    function createFungibleTokenWithSECP256K1AdminKeyPublic(address treasury, bytes memory adminKey)
        public
        payable
        returns (address)
    {
    ...
    function createFungibleTokenWithSECP256K1AdminKeyAssociateAndTransferToAddressPublic(
        address treasury,
    ...
    function createFungibleTokenWithSECP256K1AdminKeyWithoutKYCPublic(address treasury, bytes memory adminKey)
        public
        payable
        returns (address)
    {
    ...
    function createFungibleTokenWithCustomFeesPublic(address treasury, address fixedFeeTokenAddress) public payable {
        ...
        (int256 responseCode, address tokenAddress) = HederaTokenService.createFungibleTokenWithCustomFees(
    ...
    function createNonFungibleTokenWithSECP256K1AdminKeyPublic(address treasury, bytes memory adminKey) public payable {
        IHederaTokenService.TokenKey[] memory keys = new IHederaTokenService.TokenKey[](6);
        keys[0] = getSingleKey(KeyType.ADMIN, KeyType.PAUSE, KeyValueType.SECP256K1, adminKey);
        keys[1] = getSingleKey(KeyType.KYC, KeyValueType.SECP256K1, adminKey);
        keys[2] = getSingleKey(KeyType.FREEZE, KeyValueType.SECP256K1, adminKey);
    ...
    function createNonFungibleTokenWithSECP256K1AdminKeyWithoutKYCPublic(address treasury, bytes memory adminKey)
        public
        payable
    {
        IHederaTokenService.TokenKey[] memory keys = new IHederaTokenService.TokenKey[](4);
    ...
    function mintTokenToAddressPublic(address token, int64 amount, bytes[] memory metadata)
        public
        returns (int256 responseCode, int64 newTotalSupply, int64[] memory serialNumbers)
    {
        (responseCode, newTotalSupply, serialNumbers) = HederaTokenService.mintToken(token, amount, metadata);
    ...
    function associateTokenPublic(address account, address token) public returns (int256 responseCode) {
