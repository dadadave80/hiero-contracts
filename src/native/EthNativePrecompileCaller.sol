...
contract EthNativePrecompileCaller {
    ...
    event PrecompileResultUint(uint256 result);
    ...
    function call0x05(uint64 base, uint64 exp, uint64 modulus) external returns (bytes memory) {
        bytes memory fixed32BytesCalldata =
            abi.encode(abi.encodePacked(base).length, abi.encodePacked(exp).length, abi.encodePacked(modulus).length);
        bytes memory dynamicCallData = abi.encodePacked(base, exp, modulus);
