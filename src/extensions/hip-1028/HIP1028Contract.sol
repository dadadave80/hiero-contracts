...
contract HIP1028Contract is HederaTokenService, KeyHelper, FeeHelper {
    ...
    function createTokenWithMetadataAndCustomFees(string memory metadata)
        public
        payable
        returns (address createdAddress)
    {
        ...
        (int256 responseCode, address tokenAddress) =
            HederaTokenService.createFungibleTokenWithCustomFees(token, 100, 8, fixedFees, fractionalFees);
    ...
    function createTokenWithMetadataAndKey(string memory metadata) public payable returns (address createdAddress) {
        ...
        IHederaTokenService.TokenKey memory tokenKey =
            super.getSingleKey(KeyType.METADATA, KeyValueType.CONTRACT_ID, address(this));
    ...
    function createTokenWithMetadataAndKeyAndCustomFees(string memory metadata)
        public
        payable
        returns (address createdAddress)
    {
        ...
        IHederaTokenService.TokenKey memory adminKey =
            super.getSingleKey(KeyType.ADMIN, KeyValueType.CONTRACT_ID, address(this));
    ...
    function createNftWithMetadata(string memory metadata) public payable returns (address createdAddress) {
        ...
        token.treasury = address(this);
        token.tokenKeys = new IHederaTokenService.TokenKey[](1);
        IHederaTokenService.TokenKey memory tokenSupplyKey =
            super.getSingleKey(KeyType.SUPPLY, KeyValueType.CONTRACT_ID, address(this));
    ...
    function createNftWithMetadataAndCustomFees(string memory metadata)
        public
        payable
        returns (address createdAddress)
    {
    ...
    function createNftWithMetaAndKeyAndCustomFees(string memory metadata)
        public
        payable
        returns (address createdAddress)
    {
        ...
        IHederaTokenService.TokenKey memory tokenMetaKey =
            super.getSingleKey(KeyType.METADATA, KeyValueType.CONTRACT_ID, address(this));
        IHederaTokenService.TokenKey memory tokenSupplyKey =
            super.getSingleKey(KeyType.SUPPLY, KeyValueType.CONTRACT_ID, address(this));
        IHederaTokenService.TokenKey memory adminKey =
    ...
    function getInformationForToken(address token) external returns (IHederaTokenService.TokenInfo memory tokenInfo) {
        (int256 responseCode, IHederaTokenService.TokenInfo memory retrievedTokenInfo) =
    ...
    function getInformationForFungibleToken(address token)
        external
        returns (IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo)
    {
        (int256 responseCode, IHederaTokenService.FungibleTokenInfo memory retrievedTokenInfo) =
            HederaTokenService.getFungibleTokenInfo(token);
    ...
    function getInformationForNonFungibleToken(address token, int64 serialNumber)
        external
        returns (IHederaTokenService.NonFungibleTokenInfo memory nonFungibleTokenInfo)
    {
        (int256 responseCode, IHederaTokenService.NonFungibleTokenInfo memory retrievedTokenInfo) =
