...
contract CryptoOwner is HederaAccountService {
    ...
    function cryptoTransfer(address _cryptoAllowance, int64 amount, address receiver)
        external
        returns (int64 responseCode)
    {
        ...
        accountAmounts[0] =
            IHederaTokenService.AccountAmount({accountID: address(this), amount: amount * -1, isApproval: false});

        accountAmounts[1] = IHederaTokenService.AccountAmount({accountID: receiver, amount: amount, isApproval: false});

        // prepare IHTS.TransferList
        IHederaTokenService.TransferList memory transferList =
            IHederaTokenService.TransferList({transfers: accountAmounts});
        ...
        (bool success, bytes memory data) = _cryptoAllowance.call(
            abi.encodeWithSelector(ICryptoAllowance.cryptoTransferPublic.selector, transferList, tokenTransferList)
        );
