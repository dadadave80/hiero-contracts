...
contract SafeOperations is SafeHTS {
    ...
    function safeTransferNFTsPublic(
        address token,
    ...
    function safeCryptoTransferPublic(
    ...
    function safeMintTokenPublic(address token, int64 amount, bytes[] memory metadata)
        external
    ...
    function safeBurnTokenPublic(address token, int64 amount, int64[] memory serialNumbers)
        external
    ...
    function safeCreateFungibleTokenWithCustomFeesPublic(address feeCollector, address existingTokenAddress)
        external
        payable
    ...
    function safeCreateNonFungibleTokenPublic() external payable returns (address tokenAddress) {
        IHederaTokenService.Expiry memory expiry = IHederaTokenService.Expiry(0, msg.sender, 8000000);
    ...
    function safeCreateNonFungibleTokenWithCustomFeesPublic(address feeCollector, address existingTokenAddress)
        external
        payable
        returns (address tokenAddress)
    {
        ...
        (tokenAddress) =
            safeCreateNonFungibleTokenWithCustomFees(token, new IHederaTokenService.FixedFee[](0), royaltyFees);
        emit TokenCreated(tokenAddress);
        emit ResponseCode(HederaResponseCodes.SUCCESS);
    }
    ...
    function createRoyaltyFee(int32 numerator, int32 denominator, address feeCollector)
        internal
        pure
    ...
    function createFixedFeesWithAllTypes(int32 amount, address tokenId, address feeCollector)
        internal
        pure
    ...
    function createFixedFeeForToken(int32 amount, address tokenId, address feeCollector)
        internal
        pure
