...
import "../../schedule-service/IHRC1215ScheduleFacade.sol";
...
contract HRC1215Contract is HederaScheduleService {
    ...
    function scheduleCallDirect(address _to, uint256 _expiry, uint256 _gasLimit, uint64 _value, bytes memory _calldata)
        external
        returns (int64 responseCode, address scheduleAddress)
    {
        (responseCode, scheduleAddress) =
            scheduleCall(_to, _expiry > 0 ? _expiry : block.timestamp + 5, _gasLimit, _value, _calldata);
    ...
    function scheduleCallWithPayerDirect(
        address _to,
        address _payer,
        uint256 _expiry,
        uint256 _gasLimit,
        ...
        (responseCode, scheduleAddress) = scheduleCallWithPayer(
            _to, _payer, _expiry > 0 ? _expiry : block.timestamp + 5, _gasLimit, _value, _calldata
        );
    ...
    /// address, with a payer for the scheduled transaction, expiration time, the gas limit for the future call,
    /// the value to send with that call and the call data to use.
    function executeCallOnPayerSignatureDirect(
        address _to,
        address _payer,
        uint256 _expiry,
        uint256 _gasLimit,
    ...
    function deleteScheduleDirect(address _scheduleAddress) external returns (int64 responseCode) {
    ...
    function deleteScheduleProxy(address _scheduleAddress) external returns (int64 responseCode) {
    ...
    function hasScheduleCapacityProxy(uint256 _expirySecond, uint256 _gasLimit)
        external
        view
